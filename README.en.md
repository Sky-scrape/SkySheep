...
- **v1.6 (✅ current release)**: multi-instance support (source tree runs a `dev` identity isolated from the installed app), structured logging (per-session turn / tool / permission timings), bounded auto-reconnect for MCP (no replay of the failed call), shell-chaining detection per actual shell (plugs the cmd.exe single-quote & `%VAR%` bypasses), full MCP annotations on all 31 built-in tools (listed in the M8ven Trust Index), merged streaming deltas, chunked long-history rendering, and a real-socket end-to-end test layer
