...
桌面 Agent，安全是第一位的，所以我们把三件事做成了默认行为：
...
- 🧩 **MCP 与技能扩展**：MCP 客户端（stdio/HTTP，Claude Desktop 配置兼容 + 常用预设一键添加）；
  [Skills 技能包](skills-gallery/)（全局/项目两级 + [技能广场](market/)一键安装，可关键词搜索）
...
- 写文件、画图、执行命令默认弹确认：`允许一次 / 本项目总是允许 / 拒绝`；支持
  「✎ 自动写入」分级权限模式（执行命令始终仍需确认）
- 命令白名单按**命令前缀**匹配，规则按项目持久化
- `web_fetch` 仅允许公网 http(s)：解析 IP 非公网直接拒绝（防 SSRF），重定向逐跳复检
...
| Skills 技能包（SKILL.md，渐进披露） | ✅ | — | ✅ | ✅ 全局/项目两级，文件夹/zip/网址安装 + 技能广场（可搜索） |
...
| 联网抓取 + 联网搜索 | ✅ | ✅ | ✅ | ✅ web_fetch（SSRF 防护）+ web_search（博查/Tavily/智谱） |
| 文档阅读 + 生成 | ✅ | — | ✅ | ✅ read_document（PDF/Word/Excel）+ write_document（docx/xlsx/csv） |
| 图片输入 / 图片生成 | ✅ | ✅ | ✅ | ✅ 粘贴/拖拽多模态 + CogView/Kolors 画图 |
| 电脑控制（Computer Use） | — | — | — | ✅ screenshot/mouse/keyboard/window/clipboard（默认关，确认制 + 动作白名单） |
