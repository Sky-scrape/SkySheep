...
- 🧩 **MCP 与技能扩展**：MCP 客户端（stdio/HTTP，Claude Desktop 配置兼容 + 常用预设一键添加）；
  [Skills 技能包](skills-gallery/)（全局/项目两级 + [技能广场](market/)一键安装 + 「扫描本机」识别
  Claude Code 等工具已装的技能勾选复用；独立技能页可设使用范围、可搜索）
...
- 💬 **聊天机器人渠道**：把 Telegram / 微信接成遥控端，在聊天窗口里对话与审批（默认关闭；
  允许名单为空即拒绝一切，非名单内消息不回复；不开启审批时机器人只能读不能写，
  开启后敏感操作推确认卡片、超时自动拒绝。Telegram 填 Bot Token，微信扫码登录）
...
| 聊天机器人渠道（Bot Channel） | — | — | — | ✅ Telegram（Bot Token）+ 微信（扫码登录）；默认关闭，空名单拒绝一切，无人值守只读，可开审批 + 超时自动拒绝 |
...
- **v2.2（✅ 当前版本）**：多实例并存（源码版 dev 身份与安装版隔离并行）、结构化日志
...
- **v2.1（✅）**：启动器「双击打不开」连环锁根治（非阻塞聚焦 / 卡死接管 / 窗口看门狗）
